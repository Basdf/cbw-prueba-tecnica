...
from app.domains.models.errors import WorkerError
...
class TaskCeleryWorker(TaskWorker):
    def report(self, payload: TaskReportRequest) -> AsyncTaskResponse:
        try:
            ...
            log.error(f"Worker error in report: {e}")
            raise WorkerError(f"Worker error in report: {e}")
    ...
    def review_task_status(self, id: MongoObjectId) -> AsyncTaskResponse:
        try:
            ...
            log.error(f"Worker error in review_task_status: {e}")
            raise WorkerError(f"Worker error in review_task_status: {e}")
